...
class XetraETL():
    ...
    def transform_report1(self,data_frame: pd.DataFrame):
        """
        Applies transformations to create ETL_Report1

        :param Dataframe: Pandas dataframe as input

        :returns dataframe: Transformed pandas dataframe as output
        """

        if data_frame.empty:
            self._logger.info('Dataframe is empty.No transformations will be applied')
            return data_frame
        self._logger.info('Application of tranformations for Xetra source data for Report 1 started ...')
